...
import { GeoJsonTiler } from "@here/harp-mapview";
...
export class TilerService extends WorkerService {
    ...
    private async handleRegisterIndexRequest(
        ...
        const geoJsonPath = typeof message.input === "string"
            ? new URL(message.input)
            : message.input;
        await this.tiler.registerIndex(message.id, geoJsonPath);
    ...
    private async handleUpdateIndexRequest(
        ...
        this.tiler.updateIndex(message.id, geoJsonPath);
